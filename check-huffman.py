...
def huffman_tree(freq_tup):
    ...
    def internal(sorted_tup):
        ...
        sorted_tup.sort(key = lambda tup: tup[0])  #changed sort 
        least_freq = tuple(sorted_tup[0:2])# combining least frequent into a tup
        remainder = sorted_tup[2:]        # all the others
        comb_freq = least_freq[0][0] + least_freq[1][0]  # root of least frequent node
...
def generate_compressed(text, codes):
   ...
   """
    ...
    for start in range(0, end_point, 8):
        stop = start + 8
        comp_list.append(comp_text[start:stop])
...
def tree_to_bytes(tree):
    ...
    def internal(tree):
        ...
        if not tree.left and not tree.right:
            lst.append(0)
            lst.append(tree.symbol)
        (internal(tree.left), internal(tree.right))
        if not tree.symbol:
            lst.append(1)
            lst.append(tree.number)
...
def generate_tree_general(node_lst, root_index):
    ...
    def create_tree(tree):
        ...
        """
        if tree.symbol: # alternative to checking for empty list
            return tree
    
        if not tree.left.symbol: #left subtree is not a leaf
            tree.left = duplicate_node(huffman_lst[tree.left.number])
            
        if not tree.right.symbol: #right subtree is not a leaf
    ...
    def traversal(tree):
        ...
        if not huffman_lst == []:
            if not tree.left.symbol:
                ...
                if not huffman_lst == []:
                    if not tree.left.left.symbol or not tree.left.right.symbol:
                            traversal(tree.left)              
            if not tree.right.symbol:
                tree.right = huffman_lst.pop(0)
                if not huffman_lst == []:
                    if not tree.right.left.symbol or not tree.right.right.symbol:
...
def generate_uncompressed(tree, text, size):
    ...
    text = list(text)
    node = tree
    def traverse(node, new_bit, one_bit):
            if one_bit == '' or len(lst_byte) == size:
                return None
            if new_bit == '0':
                if node.left.left and node.left.right: #if its not a leaf
                    traverse(node.left, one_bit[0], one_bit[1:])
                else: #if its a leaf
                    lst_byte.append(node.left.symbol)
                    traverse(tree, one_bit[0], one_bit[1:])
            elif new_bit == '1':
                if node.right.left and node.right.right: #checks if node.right is a leaf node
                    traverse(node.right, one_bit[0], one_bit[1:]) #slice one_bit when read
                else:
                    lst_byte.append(node.right.symbol)
                    traverse(tree, one_bit[0], one_bit[1:])
    for byte in text:
        one_bit = byte_to_bits(byte) #doing it byte by byte so trees are smaller
        traverse(tree, one_bit[0], one_bit[1:])
